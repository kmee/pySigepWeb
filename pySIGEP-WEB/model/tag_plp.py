...
class TagPLP(TagBase):
    ...
    def __init__(self, cartao_postagem, id_lpl=0, valor_global=0.00,
    ...
    def xml(self):
        xml = u'<plp>'
        xml += u'<id_plp>%s</id_plp>' % str(self.id_lpl) if self.id_lpl else ''
        xml += u'<valor_global>%s</valor_global>' % str(self.valor_global) if\
            self.valor_global else ''
